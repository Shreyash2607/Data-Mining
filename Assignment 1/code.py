...
root= tk.Tk()
root.title("Assignment No. 1")
root.geometry("900x900")
lbl = Label (root, text="Assignment No 1",  fg="black", font="none 20 bold").grid(row=0, column=0, columnspan=2, pady=10, padx=10, ipadx=50)
...
def get_excel():
	...
	get_excel.data = pd.read_csv(import_file_path)

def findMean():
	data = get_excel.data["col1"].tolist()
	print(data)
	n = len(data) 
	get_sum = sum(data) 
	mean = get_sum / n 
	ans="Mean is: " + str(mean)
	w = Label(root, text=ans)
	w.grid(row=26 , column=3, columnspan=2, pady=20, padx=20, ipadx=25)
	print(ans)
