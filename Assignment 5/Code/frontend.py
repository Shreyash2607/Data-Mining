...
#from Code.backend import naive_bayes
from backend import *
window = Tk()
...
rowVal+=1
GR = Button(leftframe, text="Naive Bayes Classifier",command=naive_bayes)
GR.grid(column=0, row=rowVal, padx=2, pady=2)
rowVal+=1
GI = Button(leftframe, text="k-NN classifier",command=knn)
GI.grid(column=0, row=rowVal, padx=2, pady=2)
rowVal+=1
ANN = Button(leftframe, text="Three layer ANN",command=ANN)
