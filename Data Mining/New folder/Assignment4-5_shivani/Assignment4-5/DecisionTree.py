...
root = Tk()
root.title("Data Mining : Assignment 4")

Console = Text(root, height=1000, width=1000)
# Console.place(height=1000, width=1000, rely=0.0, relx= 0.25)
Console.pack()
...
def print_tree(node, spacing=""):
    ...
    # Base case: we've reached a leaf
    if isinstance(node, Leaf):
        write(spacing + "Leaf id: " + str(node.id) + " Predictions: " + str(node.predictions) + " Label Class: " + str(node.predicted_label))
        return

    # Print the question at this node
    write(spacing + str(node.question) + " id: " + str(node.id) + " depth: " + str(node.depth))
